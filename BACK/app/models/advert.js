...
class Advert {
    ...
    static async findMostRecent() {
            const query = {
                text: ` SELECT advert.*, localisation.department, localisation.postal_code, localisation.city, "user".pseudo FROM advert 
                JOIN localisation ON localisation.id = advert.localisation_id
                JOIN "user" ON "user".id = advert.user_id
            ...
            try {
                ...
                rows[0].postalCode = rows[0].postal_code;
                ...
                return {
                    advert: new Advert({
                        id: rows[0].id,
                        title: rows[0].title,
                        advertImage: rows[0].advert_image,
                        publicationDate: rows[0].publication_date,
                        locationPrice: rows[0].location_price
                    }),
                    user: new User({
                        pseudo: rows[0].pseudo
                    }),
