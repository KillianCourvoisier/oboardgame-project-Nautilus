...
const auth = (store) => (next) => (action) => {
  ...
  switch (action.type) {
    case LOGIN:
      axios.post('http://localhost:6050/api/login', JSON.stringify({
      ...
      break;
    case SIGN_IN: {
      axios.post('http://localhost:6050/api/signup', JSON.stringify({
    ...
    case UPDATE_PROFIL:
      axios.patch('http://localhost:6050/api/profil', JSON.stringify({
        firstname: state.user.firstname,
        lastname: state.user.lastname,
        pseudo: state.user.pseudo,
        email: state.user.email,
      ...
      break;
    case DELETE_PROFIL:
      axios.delete('http://localhost:6050/api/profil')
        .then((response) => {
          localStorage.clear();
          console.log('response', response);
    ...
    case DELETE_ADVERT:
      axios.delete('http://localhost:6050/api/adverts/:id')
        .then((response) => {
          console.log('response', response);
      ...
      break;
    case CONTACT:
      axios.post('http://localhost:6050/api/contactus', JSON.stringify({
        firstname: state.user.firstname,
        lastname: state.user.lastname,
        email: state.user.email,
      ...
      break;
    case ADVERT_CONTACT:
      axios.post('http://localhost:6050/api/mailing', {
